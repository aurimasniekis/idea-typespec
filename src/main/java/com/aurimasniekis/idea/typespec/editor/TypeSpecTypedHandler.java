...
public class TypeSpecTypedHandler extends TypedHandlerDelegate {
  @NotNull
  @Override
  public Result beforeCharTyped(char c,
    ...
    if (fileType == TypeSpecFileType.INSTANCE) {
      if (c == '\'' || c == '"' || c == '`') {
  ...
  public @NotNull Result charTyped(char c, @NotNull Project project, @NotNull Editor editor, @NotNull PsiFile file) {
    if (file.getFileType() == TypeSpecFileType.INSTANCE) {
      if (c == '\'' || c == '"' || c == '`') {
        if (!CodeInsightSettings.getInstance().AUTOINSERT_PAIR_QUOTE) {
          return Result.CONTINUE;
        }
  ...
  private static TextMateAutoClosingPair findSingleCharSmartTypingPair(char closingChar, @Nullable TextMateScope currentSelector) {
    if (!TextMateService
      .getInstance().getPreferenceRegistry().isPossibleRightSmartTypingBrace(closingChar)) {
      return null;
    }
